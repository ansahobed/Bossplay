...
export function Gallery() {
  ...
  const galleryGridRef = useRef<HTMLDivElement | null>(null); // 👈 Add ref
  ...
  const handleViewAll = () => {
    ...
    }, 100); // slight delay to allow grid render
  };

  const imagesToShow = showFullGallery ? galleryImages : galleryImages.slice(0, 8);
  ...
  return (
    <section id="gallery" className="section bg-primary">
      <div className="container">
        ...
        {/* Slider (only if not full gallery) */}
        {!showFullGallery && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
        ...
        {/* Grid Section */}
        <motion.div
          ref={galleryGridRef} // 👈 Scroll to this
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
